...
#define OMAP3_BEAGLE_TS_GPIO	42
...
/* Tandenghua */
static struct spi_gpio_platform_data beagle_spi_gpio_pdata = {
	.sck		= 17,
	.mosi		= 14,
	.miso		= 15,
	.num_chipselect	= 2,
...
/* Tandenghua */
static struct platform_device beagle_spi_gpio = {
	.name		= "spi_gpio",
	.id		= 3,
	.dev		= {
		.platform_data	= &beagle_spi_gpio_pdata,
	},
};
...
static void __init beaglefpga_init_spi(void)
{
	...
	platform_device_register(&beagle_spi_gpio);
}
...
//	OMAP3_MUX(MCSPI1_CLK, OMAP_MUX_MODE4 | OMAP_PIN_OUTPUT),	/*171*/
//	OMAP3_MUX(MCSPI1_SIMO, OMAP_MUX_MODE4 | OMAP_PIN_OUTPUT),
//	OMAP3_MUX(MCSPI1_SOMI, OMAP_MUX_MODE4 | OMAP_PIN_INPUT),
...
{
	unsigned long cs_mem_base1, cs_gpmc = 5;
	...
	ads7846_dev_init();		/* 触摸屏 */
	beagle_display_init();

#if 0
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG1));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG2));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG3));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG4));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG5));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG6));
	printk("CONFIG1: %08x\n", gpmc_cs_read_reg(cs_gpmc, GPMC_CS_CONFIG7));
#endif

	if (gpmc_cs_request(cs_gpmc, SZ_16M, &cs_mem_base1) < 0) {
		printk(KERN_ERR "Failed request for GPMC mem for test cs 5\n");
		return;
	}

//	printk (" D0=%x D1=%x D2=%x D3=%x GPIO40=%d\n",
//			omap_mux_get_gpio_offset(0x5ac),
//			omap_mux_get_gpio_offset(0x5ae),
//			omap_mux_get_gpio_offset(0x5b0),
///			omap_mux_get_gpio_offset(0x5b2),
//			omap_mux_get_gpio_offset(0x056)
//		   );
	//	gpio_request(15, "testing spi");
	//	gpio_direction_input (15);
	gpio_request(43, "PHASE ARRAY RESET");
	gpio_direction_output (43, 0);
	gpio_request(61, "NORMAL ARRAY RESET");
